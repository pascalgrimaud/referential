...
class CountryGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Country entity")
        ...
        .repeat(2) {
            ...
            .get("/referential/api/countries")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new country")
            .post("/referential/api/countries")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "name":"SAMPLE_TEXT"
                , "codeAlpha2":"SAMPLE_TEXT"
                , "codeAlpha3":"SAMPLE_TEXT"
                , "continent":"SAMPLE_TEXT"
                , "indicatif":"SAMPLE_TEXT"
                , "source":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            ...
            .repeat(5) {
                ...
                .get("/referential${new_country_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created country")
            .delete("/referential${new_country_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
